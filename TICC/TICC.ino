...
void loop() {
  ...
  while (1) {
    ...
    if( (micros() - last_micros) > 150 ) {  // hard coded to avoid fp math; breaks if PICTICK_PS changes
    ...
    for(i = 0; i < ARRAY_SIZE(channels); ++i) {
      ...
      // Only need to do anything if INTB is low, otherwise no work to do.
       if(digitalRead(channels[i].INTB)==0) {
         ...
         channels[i].ts = ((channels[i].PICstop * (int64_t)PICTICK_FS))/1000 - channels[i].tof;
